...
@interface BlindsVerticalBackMesh() {
...
@end
@implementation BlindsVerticalBackMesh
                           ...
                           direction:(BlindsDirection)direction
{
    ...
    _yResolution = (GLfloat)screenHeight / rowCount;
    _width = screenWidth;
...
{
    if (self.direction == BlindsDirectionBottomToTop) {
        rotation -= M_PI_4;
    } else {
        rotation += M_PI_4;
    }
    ...
    for (int i = 0; i < columnCount; i++) {
        ...
        if (self.direction == BlindsDirectionTopToBottom) {
            vertices[i * 4].position.y = yCenter + self.yResolution * sqr2Over2 * sin(rotation);
            vertices[i * 4].position.z = zAnchor + self.yResolution * sqr2Over2 * cos(rotation);
        } else {
        ...
        if (self.direction == BlindsDirectionTopToBottom) {
            vertices[i * 4 + 2].position.y = yCenter + self.yResolution * sqr2Over2 * cos(rotation);
            vertices[i * 4 + 2].position.z = zAnchor - self.yResolution * sqr2Over2 * sin(rotation);
        } else {
